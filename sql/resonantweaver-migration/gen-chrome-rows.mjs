...
const sql = [
  ...
  "",
  "  -- (f) nothing else on the fleet moved. Chrome is per-site; a stray row keyed",
  "  --     to another tenant would mean this file wrote outside its own site.",
  "  SELECT count(*) INTO n FROM public.content_overrides",
  "   WHERE key IN ('navLayers','footerLayers')",
  "     AND site IS DISTINCT FROM 'resonantweaver'",
  "     AND updated_at > now() - interval '1 minute';",
  "  IF n <> 0 THEN RAISE EXCEPTION 'assert: this run touched % row(s) of another site', n; END IF;",
  "",
