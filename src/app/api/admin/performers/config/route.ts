// /api/admin/performers/config — Office operator control for the @tgv/module-performers killswitch.
//
...
//
// Unlike the wallet config (tenant-owned money → proxied to tgv.com), performers enablement is
// cross-tenant OPERATOR config, so Office owns the file directly (see lib/performers-config.ts). Each
// host's performers dispatcher reads the same file via @tgv/module-performers's isPerformersEnabled().
//
...
import {
  type PerformersEnablementConfig,
  PERFORMERS_CONFIG_PATH,
  readPerformersConfig,
  writePerformersConfig,
} from "@/lib/performers-config";
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ config: readPerformersConfig() });
}

type PutBody = {
  globalKillswitch?: boolean;
  tenant?: { siteId: string; enabled?: boolean };
};
...
export async function PUT(req: NextRequest) {
  ...
  const fileExists = existsSync(PERFORMERS_CONFIG_PATH);
  const current = readPerformersConfig();
  const next: PerformersEnablementConfig = {
  ...
  if (typeof body.globalKillswitch === "boolean" && body.globalKillswitch !== current.globalKillswitch) {
    ...
    action = body.globalKillswitch ? "performers.killswitch_on" : "performers.killswitch_off";
    note = `Global performers killswitch ${body.globalKillswitch ? "ENGAGED — all tenants blocked" : "released"}`;
    ...
    if (typeof body.tenant.enabled === "boolean" && body.tenant.enabled !== cur.enabled) {
      ...
      action = body.tenant.enabled ? "performers.tenant_enabled" : "performers.tenant_disabled";
      note = `Performers ${body.tenant.enabled ? "enabled" : "disabled"} for ${cur.label ?? id}`;
    }
  }
  ...
  writePerformersConfig(next);

  await db.insert(schema.adminAuditLog).values({
    actorUserId,
    action,
    targetType: "performers_config",
    targetId,
