...
// emailed to the admin. Body: { targetMemberId, scopes[], note? }. Operator-only.
...
export async function POST(req: NextRequest) {
  ...
  if (!targetMemberId) return NextResponse.json({ ok: false, error: "targetMemberId required" }, { status: 400 });
  ...
  try {
    const res = await createConsentRequest(
      db as never,
      { kind: "access_grant", requesterMemberUserId: adminMemberUserId, targetMemberId, scopes, note },
      OFFICE_CONSENT_MAIL,
    );
